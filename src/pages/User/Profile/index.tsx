import {
    ...
    updateUserUsingPOST,
    userLoginUsingPOST,
    getSecretKeyUsingGET
} from '@/services/faiz-api-backend/userController';
...
import {uploadImgUsingPOST} from "@/services/faiz-api-backend/fileController";
...
const Profile: React.FC = () => {
    ...
    // 显示秘钥
    const showSecretKey = async (id:any) => {
        ...
        if (res.code === 0) {
            const result=await getSecretKeyUsingGET({id});
            setData((prevData) => ({
                ...prevData,
    ...
    const handleChange: UploadProps['onChange'] = (info: UploadChangeParam<UploadFile>) => {
        ...
        if (info.file.status === 'done') {
            ...
            // uploadImgUsingPOST({file: info.file.originFileObj as any}).then(res => {
            //     console.log(res)
    ...
    const resetSecretKey = async () => {
        try {
            ...
            if (res.code === 0) {
                const res = await updateKeyUsingPOST({
                    id: data?.id,
                });
                if (res.data) {
                    getUserInfo(data?.id);
    ...
    return (
        <PageContainer>
                    ...
                    <Card title="秘钥操作" bordered={false}>
                        <Row>
                            <Col>
                                ...
                                >
                                    <LockOutlined/> accessKey：{(data?.accessKey == '' || data?.accessKey == null) ? '暂无密钥，请点击"重置密钥"按钮获得密钥' : data?.accessKey}
                                </Paragraph>
                            </Col>
                        </Row>
                        <Divider/>
            ...
            <Modal
                title="查看秘钥"
                open={open}
                onOk={flag ? ()=>showSecretKey(data?.id) : resetSecretKey}
